...
1. **Sender name mismatch**: If the person sending the funds differs from the investor named on the notification, the transfer will be rejected
2. **Incorrect transfer purpose description**: When initiating the bank transfer, you must clearly specify the purpose as "foreign investment funds" or equivalent
3. **Exchange rate differences**: The funds are converted to KRW at the exchange rate on the date of transfer, which can create a discrepancy between the notified amount and the actual deposit
